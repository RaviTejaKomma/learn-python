...
def test_list_len():
    fruits = ["banana", "orange", "grape"]
    assert 3 == len(fruits)                     # len() returns an integer
...
def test_list_is_mutable():
    fruits = ["banana", "orange", "grape"]                # lists are mutable unlike strings
    fruits[0] = "mango"
...
def test_list_slice_replacement_is_inplace():
    ...
    assert ["banana","litchi", "guava","grape"] == fruits # it inserts the new list
...
# and fix accordingly.
def test_list_membership():              #DOUBTTTTTTTTTTTTTTTTTTTTTTT CLARIFIED
    numbers = [ 5, 4, 3]                 # take a note of this
    assert 5 == 5 in numbers
    assert  True or 10 == 10 in numbers
...
def odd_desc(count):
    ...
    """
    return list(reversed(range(1,count*2,2))) # we can't use reverse() because it returns none

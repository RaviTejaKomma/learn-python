...
def test_none_type():
    assert 'NoneType' == type(None).__name__            # it his NoneType
...
def test_truth_none():
    value = None
    result = "not-set"
    #is None treated as true or false?                # None is treated as False
    if value:
        result = "true"
    else:
        result = "false"
...
def test_equality():
    ...
    assert True== (0 == False) # what happened here?  ########### 0 is boolean it can compared with True or False
    assert True== (1 == True)
    assert False == ("a" == True)
...
- i have learnt about the truth values of non boolean types
- i have learnt about truth values of certain datatypes in controlflow
- i have learnt that The fact that certain things are treated as True or False by
  control flow statements does not mean that they are equal to True or False.
